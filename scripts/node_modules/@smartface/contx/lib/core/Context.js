...
class Context {
  constructor(actors, reducer, initialState = {}, hookFactory = null) {
    ...
    this.state = Object.assign({}, initialState); // new
  ...
  map(fn) {
    ...
    return res; // return this.actors.$$map.map((instance, index) => {
  ...
  add(actor, name) {
    ...
    this.actors.collection.set(instance, actor); // this.actors.$$idMap[actor.getID()] = instance;
  ...
  removeChildren(instance) {
    const removeActor = this.actors.collection.get(instance); //this.actors.collection.delete(instance);
  ...
  remove(instance) {
    ...
    const actor = this.actors.collection.get(instance); // console.log("remove actor: "+actor.getInstanceID());

    if (actor) {
      this.actors.collection.delete(instance); // console.log("before remove actor : "+JSON.stringify(this.actors.$$map));
      // this.actors.$$map = Object.keys(this.actors.collection);
  ...
  dispatch(action, target) {
    try {
      ...
      (0, _raiseErrorMaybe.default)(e, target && !!this.actors.collection[target] && (e => this.actors.collection[target].onError(e)));
    }
  }
...
_defineProperty(Context, "getID", function () {
  var ID = 1;
  return () => ++ID;
}());
