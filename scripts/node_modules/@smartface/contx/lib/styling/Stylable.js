...
var _application = _interopRequireDefault(require("sf-core/application"));

var _findClassNames2 = _interopRequireDefault(require("@smartface/styler/lib/utils/findClassNames"));

var _toStringUtil = _interopRequireDefault(require("../util/toStringUtil"));

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

const _findClassNames = classNames => (0, _findClassNames2.default)(classNames).reduce((acc, item) => !item && [] || [...acc, item.join('')], []);
...
function makeStylable({
  component,
  classNames = "",
  userStyle = {},
  name
}) {
  userStyle = (0, _merge.default)(userStyle);
  /**
   * Styable actor
   * @class
   */
...
class Stylable extends _Actor.default {
  ...
  getUserStyle() {
    return (0, _merge.default)(this.userStyle);
  }
  ...
  updateUserStyle(props) {
    this.userStyle = (0, _merge.default)(this.userStyle, props);
    this.isDirty = true;
    return this;
  }
  ...
  setUserStyle(props) {
    if (isFunction(props)) {
      ...
      this.userStyle = (0, _merge.default)(props);
    }
  ...
  computeAndAssignStyle(style, force = false) {
    ...
    var _component = this.getName().endsWith("_statusBar") ? _application.default.statusBar || this.component : this.component;

    const reduceDiffStyleHook = hooks("reduceDiffStyleHook") || null;
    style = (0, _merge.default)(style, this.userStyle);
    const safeAreaProps = {};
    ...
    const rawDiff = isFunction(diffReducer) ? Object.keys(style).reduce(diffReducer, {}) : (0, _merge.default)(this.styles, style);
    ...
    _component.subscribeContext ? hasDiff && _component.subscribeContext({
      ...
      rawStyle: (0, _merge.default)(rawDiff)
      ...
      try {
        ...
        e.message = "When [" + key + "] raw value : [\n" + (0, _toStringUtil.default)(style[key]) + "\n] \n is being assigned as : [\n" + (0, _toStringUtil.default)(diff[key]) + "\n\r] " + e.message;
        throw e;
      }
    }); // <-------------------

    const afterHook = hooks("afterStyleDiffAssign"); //isScrollView && console.log("diff: " + JSON.stringify(diff)+"\nRawDiff: " + JSON.stringify(rawDiff)+"\n");
  ...
  setStyles(style, force = false) {
    this.waitedStyle = (0, _merge.default)(this.waitedStyle, style);
    this.makeDirty();
    return this;
  }
